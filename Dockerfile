...
RUN mkdir -p /usr/local/src/boardblock

# Set the created directory as a Docker volume
VOLUME ["/usr/local/src/boardblock"]
...
RUN echo "pip install -r /usr/local/src/boardblock/requirements.txt" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
    echo "pip install -e /usr/local/src/boardblock" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
    echo "cd /usr/local/src/boardblock && make compile_translations && cd /usr/local/src/xblock-sdk" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
